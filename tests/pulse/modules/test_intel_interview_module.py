...
def test_intel_interview_collect_report_and_push_routes() -> None:
    ...
    assert collect_data["total"] <= 4
    assert "persisted_docs" in collect_data

    assert report_resp.status_code == 200
    report_data = report_resp.json()
    assert report_data["summary"]["report_count"] == len(report_data["items"])
    assert report_data["summary"]["report_count"] <= 3

    assert push_resp.status_code == 200
    push_data = push_resp.json()
    assert push_data["ok"] is True
    assert push_data["pushed_count"] == push_data["report"]["summary"]["report_count"]
    ...
    assert "intel_interview.daily_push" in schedule_status["tasks"]

    assert schedule_trigger_resp.status_code == 200
    trigger_data = schedule_trigger_resp.json()
    assert trigger_data["ok"] is True
    assert any(task == "intel_interview.daily_push" for task in trigger_data["ran_tasks"])
