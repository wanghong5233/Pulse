...
class PreferenceExtractor:
    ...
    def _build_instruction(text: str) -> str:
        ...
        return (
            ...
            "重复投递\"、\"秋招叙事要顺畅\".\n"
            "       - avoid_trait: 用户明确回避的**岗位/公司类型(集体名词)**. "
            "凡是用户原话出现\"不要 X / 避开 X / 不投 X / 战略性放弃 X / 暂时不考虑 X\" 且 X "
            "**不是单独一家公司**而是一个**类别标签**, 都必须输出一条 avoid_trait, "
            "target=该类别词的最简形式.\n"
            "         典型例子:\n"
            ...
            "           \"避开广告业务\"                → target=\"广告业务\"\n"
            "           \"不要国企\"                    → target=\"国企\"\n"
            "         注意: \"放弃大厂\"和\"找小厂或初创\"是**同一句话的两面**, 用户既"
            "在 avoid 又在 favor; 这两条都要输出 (avoid_trait + favor_trait).\n"
            ...
            "       - avoid_company / favor_company: 公司黑白名单(**只用于具体公司名**, "
            "比如\"拼多多不要再投\"; 集体名词如\"大厂/外包\"一律走 avoid_trait, **不要**"
            "在这里展开成具体公司列表 — 由下游 LLM matcher 用世界知识展开).\n\n"
